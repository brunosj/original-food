...
>
	<div class="flex space-y-3 lg:space-y-6 flex-col lg:text-right">
		<h1
			...
			class={$uspBackground === 'var(--color-pri)' ? 'text-ter font-bold' : 'text-sec font-bold'}
		>
			{title}
		</h1>
		<h3
	...
	<div class="hidden xl:grid grid-cols-2 grid-rows-2 gap-x-4 sectionPt text-ter">
		<a
			href="/projekte"
			class="rounded-xl row-span-2 h-[60vh] relative group cursor-pointer"
			onmouseover={() => handleMouseOver(wirtschaft.index)}
			onmouseleave={() => (expandedIndex = -1)}
		>
			<div
				class={`absolute ${
					$uspBackground === 'var(--color-pri)' ? 'bg-ter text-black' : 'bg-sec text-ter'
		...
		<a
			...
			onmouseover={() => handleMouseOver(ökologie.index)}
			onmouseleave={() => (expandedIndex = -1)}
		>
			<div
				class={`absolute ${
		...
		<a
			...
			onmouseover={() => handleMouseOver(soziales.index)}
			onmouseleave={() => (expandedIndex = -1)}
		>
			<div
				class={`absolute ${
