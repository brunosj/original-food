<script lang="ts">
	...
	let { bild, hoverBild, link, titel, untertitel, beschreibung, preis, siegel } = item.attributes;
	...
	<a href={link} class="flex flex-col relative card h-full" target="_blank">
		<div class="relative">
			...
			{#if siegelUrl}
				...
				>
					<img loading="lazy" src={siegelUrl} alt={titel} class=" h-6 w-full object-contain" />
				</div>
			{/if}
		</div>
		<div
